<?php
  ...
  //Reviso el estado de los comentarios
    if ($motivo=='') {
      $motivo='NO SE DIO MOTIVO DE LA APROBACION';
    }else{
      $motivo="MOTIVO DE LA APROBACION: $motivo";
    }
  //Defino la sentencia
    $sentencia="
      UPDATE 
  ...
  //Evaluo si se realizo la sentencia
    if ($devuelto==True) {
      echo "
      <script>
        alert('BOLETO APROBADO, RECARGA LA TABLA PARA VISUALIZAR LOS DATOS');
        $('#aprobar_boleto').modal('hide');
      </script>";
    }
  //
?>