<?php
	...
	$REDEX=$conn->prepare("SELECT id FROM usuarios WHERE clave='$clave'");
	$REDEX->execute();
	$search=$REDEX->fetch(PDO::FETCH_ASSOC);
	$dto=$search['id'];
	if (($dto==$id) || ($dto==NULL)) {
		$sql=$conn->prepare("UPDATE usuarios set clave='$clave', nombre='$nombre', apellido='$apellidos', empresa=$empresa, division=$division, correo='$correo', telefono='$telefono' where id=$id");
		$res=$sql->execute();
		if($res === TRUE){
		echo "
