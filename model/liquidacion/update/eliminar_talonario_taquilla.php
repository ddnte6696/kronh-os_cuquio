<?php
  ...
  //Obtengo el dato enviado por el formulario y lo separo
    $dto=explode("||",campo_limpiado($_POST['dato'],2,0));
  //Variables unicas
    $id_corte=$dto[0];
    $id_boleto=$dto[1];
  ...
  //Obtengo datos del talonario
    $id_talonario=busca_existencia("SELECT talonario as exist from talonarios_liquidados where id=$id_boleto");
   
	//elimino el talonario de la tabla y realizo cambios en el listado de talonarios
		$sentencia="
			DELETE FROM talonarios_liquidados where id=$id_boleto;
      UPDATE talonario set actual=actual-1, restantes=restantes+1 where id=$id_talonario;
		";
