<?php
  ...
  //Realizo la actualizacion de la liquidacion
    $sentencia="UPDATE liq_op_rl SET paqueterias_talonario=$importe where id=$id_liquidacion;";
  //Ejecuto la sentencia
    $resultado=ejecuta_sentencia_sistema($sentencia,TRUE);
  //Evaluo el resulado
    if ($resultado === true) {
      //Escribo en la bitacora
        //registra_bitacora("SE AGREGA UN IMPORTE DE $importe EN LAS PAQUETERIAS DE TAQUILLA MANUALES EN LA LIQUIDACION N° $id_liquidacion");
      ...
      //Imprimo un mensaje de confirmacion y continuo con el proceso
        echo "
          <script>
            alert('IMPORTE DE PAQUETERIAS FISICAS AGREGADO');
          </script>
        ";
      //
    }
