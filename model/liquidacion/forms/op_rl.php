...
<script>
  ...
  //Funcion para cargar el formulario de liquidacion especifico
    function buscar_fecha_liquidacion(){
      var fecha=$("#fecha").val();
      var url="model/liquidacion/queries/unidades_venta_op.php"
      $.ajax({
        type: "POST",
        url:url,
        data:{fecha:fecha},
...
<div class="card">
  <div class="card-header"><h5>REGISTRO DE LIQUIDACION DE RUTA LARGA</h5></div>
  <div class="card-body">
    ...
    <a onclick="buscar_fecha_liquidacion();" class="btn btn-sm btn-block btn-primary text-light">BUSCAR</a>
  </div>
...
<script type="text/javascript">
  /*$(function buscar_fecha_liquidacion(){
    $("#frm_buscar_fecha_liquidacion").on("submit", function(e){
      e.preventDefault();
      var f = $(this);
      var formData = new FormData(document.getElementById("frm_buscar_fecha_liquidacion"));
      formData.append("dato", "valor");
      $.ajax({
        url: "model/liquidacion/queries/unidades_venta_op.php",
        type: "post",
        dataType: "html",
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
        beforeSend: function(){ $("#respuesta_buscar_fecha_liquidacion").html("<div class='spinner-border'></div>"); },
      })
      .done(function(res){ $("#respuesta_buscar_fecha_liquidacion").html(res); });
    });
  });*/
</script>