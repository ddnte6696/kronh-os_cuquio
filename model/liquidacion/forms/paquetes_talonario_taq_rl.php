...
<script type="text/javascript">
  //Funcion para cargar el formulario de liquidacion especifico
    function muestra_talonario(opcion){
      var dato=$("#target").val();
      var url="model/liquidacion/queries/talonario_paq_taq_rl.php"
      $.ajax({
        type: "POST",
        url:url,
        data:{
  ...
  //Funcion para cargar el formulario de liquidacion especifico
    function tabla_talonario(opcion){
      var dato=$("#target").val();
      var url="model/liquidacion/queries/tabla_paq_talonario_taq_rl.php"
      $.ajax({
        type: "POST",
        url:url,
        data:{
      ...
      <tbody>
        <?php
          //Defino la sentencia a ejecutar
            $sentencia = "SELECT * FROM talonario WHERE restantes>0 and usuario='$usuario' and uso=1 and tipo=2";
          //Ejecuto la sentencia y almaceno lo obtenido en una variable
            $resultado_sentencia = retorna_datos_sistema($sentencia);
          //Identifico si el reultado no es vacio
            if ($resultado_sentencia['rowCount'] > 0) {
