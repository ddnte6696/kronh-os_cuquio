<?php
  ...
  if(isset($id) || isset($fecha_fin) || isset($h_fin)){
    //Se define la sentencia de revision a ejecutar
    $sentencia="SELECT * FROM deshabilitacion where id=$id";
    //Trato de ejecutar la sentencia y sino arrojo el error
    try {
      $p1s=$conn->prepare($sentencia);
      $p1s->execute();
      $p1=$p1s->fetch(PDO::FETCH_ASSOC);
      $id_unidad=$p1['id_unidad'];
      $dto=$p1['visual'];
      if ($dto!=0) {
        $sentencia="
          UPDATE deshabilitacion set fecha_fin='$fecha_fin', hora_fin='$h_fin',visual=0 where id=$id;
